...
export const LeaveDetailsDialog: React.FC<LeaveDetailsDialogProps> = ({
    ...
    const handleAction = (action: "approve" | "reject" | "cancel") => {
        if (leaveRequest) {
            if (action === "cancel") {
                if (leaveRequest.status !== "pending" && !isEmployer) {
                    showSnackbar({
    ...
    const handleConfirmCancel = () => {
        if (leaveRequest) {
            updateLeaveRequestMutation.mutate({
                leaveRequestId: leaveRequest._id,
                action: "cancel",
                remarks: leaveRequest.status === "cancelled" ? leaveRequest.cancelReason : (remarks || "Cancelled by user"), // Use existing reason if already cancelled? No, logic is new cancel.
                documents: updatedDocuments,
            });
    ...
    return (
        <>
            ...
            >
                ...
                <DialogActions>
                    <Button onClick={() => setOpenCancelConfirm(false)} color="inherit">
                        No, Keep it
                    </Button>
