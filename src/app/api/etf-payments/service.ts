...
import EtfPayment from "@/app/models/EtfPayment";
...
// Validation schemas
export const etfPaymentCreateSchema = z.object({
...
export const etfPaymentUpdateSchema = z.object({
...
export class EtfPaymentService {
    static async createPayment(body: any, context: RequestContext) {
        await dbConnect();

        const parsedBody = etfPaymentCreateSchema.parse(body);
        ...
        const existing = await EtfPayment.findOne({
        ...
        if (existing) {
            throw new BadRequestError(
                `ETF payment already exists for period ${parsedBody.period}`
            );
        }

        // Create payment
        const payment = await EtfPayment.create({
            company: parsedBody.companyId,
            period: parsedBody.period,
    ...
    static async updatePayment(body: any, context: RequestContext) {
        ...
        const parsedBody = etfPaymentUpdateSchema.parse(body);

        const payment = await EtfPayment.findById(parsedBody.id);
        if (!payment) {
            throw new NotFoundError("ETF payment not found");
        }
        ...
        return {
            message: "ETF payment updated successfully",
            payment,
        };
    }
    ...
    static async deletePayment(paymentId: string, context: RequestContext) {
        ...
        const payment = await EtfPayment.findById(paymentId);
        if (!payment) {
            throw new NotFoundError("ETF payment not found");
        }
        ...
        await EtfPayment.findByIdAndDelete(paymentId);

        return { message: "ETF payment deleted successfully" };
    }
    ...
    static async getPayments(req: any, context: RequestContext) {
        ...
        const payments = await EtfPayment.find(query)
