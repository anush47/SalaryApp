...
import { createEpfPayment } from '@/app/lib/api';
import { useSnackbar } from '@/app/context/SnackbarContext';
import { uploadFile } from '@/app/lib/uploadService';
import { styled } from '@mui/material/styles';

interface EpfPaymentDialogProps {
    open: boolean;
    onClose: () => void;
    onSuccess: () => void;
    companyId: string;
...
export default function EpfPaymentDialog({
    open,
    onClose,
    onSuccess,
    companyId,
    initialPeriod
}: EpfPaymentDialogProps) {
    ...
    useEffect(() => {
        ...
        const empl = parseFloat(employeeContribution) || 0;
        const sur = parseFloat(surcharges) || 0;
        setTotalAmount((emp + empl + sur).toFixed(2));
    }, [employerContribution, employeeContribution, surcharges]);
    ...
    const handleSave = async () => {
        ...
        if (!employeeContribution) newErrors.employeeContribution = 'Required';
        ...
        try {
            ...
            const paymentData = {
                ...
                createdBy: "USER_ID_PLACEHOLDER" // This should be handled by backend from session
            };

            await createEpfPayment(paymentData);
            showSnackbar({ message: 'EPF Payment recorded successfully', severity: 'success' });
            onSuccess();
            onClose();
        } catch (error: any) {
        ...
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth fullScreen={isMobile}>
            <DialogTitle sx={{ borderBottom: '1px solid', borderColor: 'divider' }}>
                <Box display="flex" justifyContent="space-between" alignItems="center">
                    <Typography variant="h6">Record EPF Payment</Typography>
            ...
            <DialogContent>
                <Box sx={{ mt: 2 }}>
                    <Grid container spacing={2}>
                        ...
                        <Grid item xs={12} sm={6}>
                            <TextField
                                label="Employer Contribution (12%)"
                                type="number"
                                fullWidth
